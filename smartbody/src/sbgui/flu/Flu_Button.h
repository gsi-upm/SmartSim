...
class FLU_EXPORT Flu_Button : public Fl_Button
{
 public:
  ...
  //! Override of Fl_Widget::image()
  inline Fl_Image* image()
    { return Fl_Button::image(); }
  ...
  void down_image( Fl_Image *i );

  inline void down_image( Fl_Image &i )
    { down_image( &i ); }

  inline Fl_Image* down_image()
    { return downImg; }

  //! If enabled, when the button value is nonzero, the image is the normal image. Else it is the desaturated image. Note this overrides the down_image()
  void image_follows_value( bool b );

  //! If enabled, when the button value is nonzero, the image is the normal image. Else it is the desaturated image. Note this overrides the down_image()
  bool image_follows_value();
