// $Id: Flu_Collapsable_Group.cpp,v 1.9 2005/12/23 19:29:42 jbryan Exp $
...
Flu_Collapsable_Group :: Flu_Collapsable_Group( int x, int y, int w, int h, const char *l )
  : Fl_Group( x, y, w, h ), button( x, y, w, _BUTTON_HEIGHT ), group( x, y+_BUTTON_HEIGHT, w, h-_BUTTON_HEIGHT )
{
  _originalHeight = h;
  _changing = false;
  _collapseTime = 0.25f;
...
void Flu_Collapsable_Group :: resize( int x, int y, int w, int h )
{
  ...
  button.resize( x, y, w, _BUTTON_HEIGHT );
  group.resize( x, y+_BUTTON_HEIGHT, w, h-_BUTTON_HEIGHT );
}
