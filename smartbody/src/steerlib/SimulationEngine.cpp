...
void SimulationEngine::init(SimulationOptions * options, SteerLib::EngineControllerInterface * engineController)
{
	...
	_spatialDatabase = new GridDatabase2D(xmin, xmax, zmin, zmax, _options->gridDatabaseOptions.numGridCellsX, _options->gridDatabaseOptions.numGridCellsZ, _options->gridDatabaseOptions.maxItemsPerGridCell);
...
bool SimulationEngine::update( bool advanceRealTimeOnly, bool overrideClockTime, float newOverrideTime )
{
	// @todo
	...
	if (!advanceRealTimeOnly) {
		...
		if (_simulateOneStep(overrideClockTime, newOverrideTime) == true) {
			return true;
		} else {
...
bool SimulationEngine::_simulateOneStep( bool overrideClockTime, float newOverrideTime )
{
	unsigned int numDisabledAgents = 0;

	// TODO - did shawn accidentally remove "advanceSimulationAndUpdateRealTime"???
	float currentSimulationTime = _clock.getCurrentSimulationTime();
	float simulationDt = _clock.getSimulationDt();
	unsigned int currentFrameNumber = _clock.getCurrentFrameNumber();

	if (overrideClockTime) {
		static float previousOverrideTime = 0.0;
	...
	for ( moduleIterator = _modulesInExecutionOrder.begin(); moduleIterator != _modulesInExecutionOrder.end();  ++moduleIterator ) {
		(*moduleIterator)->preprocessFrame(currentSimulationTime, simulationDt, currentFrameNumber);
	}
	...
	for ( agentIterator = _agents.begin(); agentIterator != _agents.end(); ++agentIterator ) {
		if ((*agentIterator)->enabled()){
			(*agentIterator)->updateAI(currentSimulationTime, simulationDt, currentFrameNumber);
		}
		else {
			numDisabledAgents++;
		}
	...
	for ( moduleIterator = _modulesInExecutionOrder.begin(); moduleIterator != _modulesInExecutionOrder.end();  ++moduleIterator ) {
		(*moduleIterator)->postprocessFrame(currentSimulationTime, simulationDt, currentFrameNumber);
	}
