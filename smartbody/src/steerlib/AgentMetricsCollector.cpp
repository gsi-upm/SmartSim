...
void AgentMetricsCollector::_checkAndUpdateOneCollision(unsigned int collisionKey, float penetration, float currentTimeStamp)
{
	if (penetration > 0.0f) {
...
void AgentMetricsCollector::_updateCollisionStats(GridDatabase2D * gridDB, AgentInterface * updatedAgent, float currentTimeStamp)
{
	...
	for (neighbor = neighbors.begin(); neighbor != neighbors.end(); ++neighbor) {
		...
		unsigned long collisionKey = reinterpret_cast<unsigned long>(item);
