...
namespace SteerLib {
	...
	public:
		...
		//@{
		GridDatabase2D(float xmin, float xmax, float zmin, float zmax, unsigned int numXCells, unsigned int numZCells, unsigned int numItemsPerCell);
		GridDatabase2D(const Util::Point & origin2D, float xExtent, float zExtent, unsigned int numXCells, unsigned int numZCells, unsigned int numItemsPerCell);
		~GridDatabase2D();
		//@}
