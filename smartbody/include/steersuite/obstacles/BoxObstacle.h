...
namespace SteerLib {
	...
	{
	public:
		BoxObstacle(float xmin, float xmax, float ymin, float ymax, float zmin, float zmax) : _bounds(xmin, xmax, ymin, ymax, zmin, zmax) { _blocksLineOfSight = (ymax > 0.7) ? true : false; }
		BoxObstacle(const Util::AxisAlignedBox & newBounds ) : _bounds(newBounds) { _blocksLineOfSight = (newBounds.ymax > 0.7) ? true : false; }
