...
public:
	...
	Util::Vector velocity() { return _forward * _currentSpeed; }
	...
	bool isSelected() { 
		return PPRGlobals::gEngineInfo->isAgentSelected(this);
	}

	const SteerLib::SteeringCommand & getSteeringCommand() { return _finalSteeringCommand; }
...
protected:
	...
	bool threatListContainsAgent(PPRAgent * agent, unsigned int &index);
	inline bool threatListContainsAgent(PPRAgent * agent) { unsigned int dummy; return threatListContainsAgent(agent, dummy); }
	void disable();
