...
namespace SteerLib {
	...
	public:
		...
		std::map<unsigned int, SteerLib::CollisionInfo> * getCurrentCollisions() { return &_currentCollidingObjects; }
		/// Returns the number of total unique collisions, both past and present.
	    unsigned int getNumTotalCollisions() { return _pastCollisions.size() + _currentCollidingObjects.size(); }
	...
	protected:
	    ...
	    void _checkAndUpdateOneCollision(unsigned int collisionKey, float penetration, float currentTimeStamp);
		...
		// collision history
		std::map<unsigned int, SteerLib::CollisionInfo> _currentCollidingObjects; // a list of agents and obstacles that this agent is colliding with.  hopefully won't ever be too large.
